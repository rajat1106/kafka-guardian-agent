...
import {
  AlertTriangle, Brain, CheckCircle2, Loader2, ShieldAlert, ShieldX,
  Terminal, XCircle,
} from "lucide-react";
import { api, type IncidentDetail as Detail, type TimelineEntry } from "@/lib/api";
import {
 ...
 */
const Step = ({ entry }: { entry: TimelineEntry }) => {
          ...
          <ScrollArea className="max-h-[60vh] pr-3">
            <div className="space-y-4 border-l border-border/50 pl-3">
              ...
              {detail.timeline.map((entry, i) => (
                <Step key={`${entry.kind}-${i}`} entry={entry} />
              ))}
            </div>
